...
5. 系统按阈值把 audit memory 总结进当天 daily 记忆；每次 summary 完成后立即滚动出新的 audit 文件，用户也可在 `Memory` 页面手动点击 `Summary` 触发同样流程。用户可通过 `Calendar` 回顾完成记录，并在 `Memory` 页面查看 audit 日志、daily 记忆与长期记忆。
...
6. **Memory Pipeline（记忆管线）**：把 Web 操作、Plan 交互、Agent 事件、Terminal I/O 写入 audit 记忆；在滚动阈值触发或用户手动触发时，把当前 audit 文件总结到 daily 记忆并立即滚动出新的 audit 文件；在次日 `00:00` 之后完成 daily 定稿并提炼长期记忆。
...
- `tasks`
  - `id, public_id, goal_id, title, summary, description, status, task_type, estimated_minutes, context_key, created_at, completed_at`
  - 说明：
