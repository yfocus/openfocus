...
- `InspirationSpace`：围绕一个灵感主题的持续讨论空间，保存消息、资源、草案版本与发布结果，用于在正式创建 Goal/Tasks 之前进行探索、澄清与结构化。
...
2. 当用户还没有想清楚目标或需要进一步澄清思路时，进入 `Inspiration` 页面创建一个 `InspirationSpace`，上传资源并与内建规划 Agent 持续讨论。
3. 用户在 `InspirationSpace` 中多轮追问、补充资源、生成/修订 `Draft vN`，并在确认后正式发布为 `1 个 Goal + 多个 Tasks`。
...
1. **Inspiration Planner（灵感规划）**：`InspirationSpace` 承载持续讨论、资源引用、草案版本与发布确认；在用户点击 `Publish` 之前，不写入 Goal/Tasks。
...
- `inspiration_spaces`
  - `id, title, status, published_goal_id, forked_from_space_id, last_activity_at, message_turn_count, created_at, updated_at, closed_at, published_at`
...
- `inspiration_resources`
  - `id, space_id, resource_seq_id, type, name, text_content, url_content, file_path, is_system_generated, created_at, updated_at, deleted_at`
  - 说明：`type` 支持 `url | image | text | summary`；文本与 URL 存 DB，图片落文件系统并记录路径。
...
- 用户创建 `InspirationSpace` 后，内建规划 Agent 通过持续追问、资源引用与上下文澄清来帮助用户收敛目标。
- 当用户显式触发 `/draft_goal_tasks`，或 Agent 判断上下文已经足够完整时，可生成新的 `Draft vN`。
...
- 每次草案生成都必须保存历史版本，并在消息流中以 `Draft vN` 的 assistant 卡片形式展示。
...
- `published` 时必须生成一个只读的 `Published Summary` 资源，内容包含：`Idea`、`Why now`、`Goal`、`Published tasks`、`Open questions`、`Rejected / deferred ideas`。
...
- Inspiration（先讨论/孵化再创建）→ LLM 生成 `Draft vN`，发布前只写草案与资源，不写 Goal/Tasks
- `GET /api/recommendations/next` 返回下一步推荐（带 why）

验收：Inspiration 的 `Publish` 后能创建 Goal/Tasks，并生成 `Published Summary`；Dashboard 能显示“下一步建议”。
