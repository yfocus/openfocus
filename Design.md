...
- `Memory`：以 Markdown 形式存储的长期用户上下文（`User Card` / `User Memory`）。
...
4. Agent/Skill 运行过程中持续上报事件；用户在 Dashboard 右侧事件流或 Task 详情事件区查看进展。
5. 通过 `Calendar` 查看按月完成记录，通过 `Memory` 页面维护 `User Card` 与 `User Memory`。
...
- `Memory`：编辑 `User Card` / `User Memory` Markdown。
...
6. **Personal State**：用 Markdown 存储 `User Card` / `User Memory`，并提供 Calendar 月视图辅助回顾。
...
- Dashboard `Recent Events`：按时间倒序展示真实落库事件。
- Task 详情 `Event`：展示该 Task 最近若干条事件，便于人工确认完成前复核。
- `Calendar`：按月查看 `task.confirmed_done` 与 Goal 时间线。
- `Memory`：维护 `User Card` / `User Memory` 的 Markdown 内容。

### 11.2 延后能力

- 独立的 `History & Metrics` 页面。
- `real vs agent_sum`、token 趋势、leverage 图表。
- 基于 Run/Review/Artifact 的复盘视图。
...
- Memory 页面（User Card / User Memory Markdown）

验收：用户能按月回顾完成记录，并维护长期个人上下文。
