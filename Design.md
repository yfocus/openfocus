...
- Web UI 与 OpenFocus 同源（如 `http://127.0.0.1:8000`）。
- OpenFocus 与 Companion 同机（如 `127.0.0.1:17890`），推荐由 OpenFocus 反向代理到 Companion，避免浏览器 CORS/端口管理复杂度。
- 目录选择：Web UI 调用 OpenFocus API，OpenFocus 转发给 Companion 执行并返回绝对路径。

#### 12.3.2 多机（多台电脑/服务器）

- Companion 以出站方式连接 OpenFocus（更易穿透 NAT/防火墙），并保持长连接（WebSocket / gRPC streaming）。
- 长连接同时承载：
  - `register/hello`：注册与能力声明（capabilities）
  - `heartbeat`：保活与 last_seen（也可用 WS ping/pong）
  - `command`：OpenFocus 下发命令（带 `command_id`，幂等）
  - `event/stream`：回传进程输出、状态、文件预览摘要等（可映射为 `/api/agent/events` 事件落库）
- 断线重连：指数退避 + resume（按 last_ack / cursor 补发关键事件）。
