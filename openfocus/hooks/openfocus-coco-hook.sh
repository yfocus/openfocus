...
# OpenFocus Coco/Trae hook shim. Best-effort local signal forwarding.
...
case "$RAW_KIND" in
  PreToolUse) KIND="pre-tool-use" ;;
  PostToolUse) KIND="post-tool-use" ;;
  PostToolUseFailure) KIND="post-tool-use-failure" ;;
  UserPromptSubmit) KIND="user-prompt-submit" ;;
  Stop) KIND="stop" ;;
  SubagentStart) KIND="subagent-start" ;;
...
log_hook_error() {
  ...
  printf '%s agent=coco kind=%s sock=%s %s\n' \
...
ENVELOPE=$(printf '{"schema_version":1,"agent_runtime":"coco","hook_kind":"%s","runtime_ts":%s,"runtime":%s,"payload":%s}\n' \
