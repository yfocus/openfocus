...
# OpenFocus Claude Code hook shim. Best-effort local signal forwarding.
...
if [ -z "$RAW_KIND" ]; then
  RAW_KIND=$(printf '%s' "$PAYLOAD" | jq -r '.hook_event_name' 2>/dev/null \
...
case "$RAW_KIND" in
  SessionStart|session_start|session-start) KIND="session-start" ;;
  UserPromptSubmit|user_prompt_submit|user-prompt-submit) KIND="user-prompt-submit" ;;
  PermissionRequest|permission_request|permission-request) KIND="permission-request" ;;
  PreToolUse|pre_tool_use|pre-tool-use) KIND="pre-tool-use" ;;
  PostToolUse|post_tool_use|post-tool-use) KIND="post-tool-use" ;;
  PostToolUseFailure|post_tool_use_failure|post-tool-use-failure) KIND="post-tool-use-failure" ;;
  Stop|stop) KIND="stop" ;;
  Notification|notification) KIND="notification" ;;
  SessionEnd|session_end|session-end) KIND="session-end" ;;
  *) KIND="${RAW_KIND:-unknown}" ;;
esac
...
log_hook_error() {
  ...
  printf '%s agent=claude-code kind=%s sock=%s %s\n' \
...
ENVELOPE=$(printf '{"schema_version":1,"agent_runtime":"claude-code","hook_kind":"%s","runtime_ts":%s,"runtime":%s,"payload":%s}\n' \
