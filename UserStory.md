...
**概述**
`What's Next` is the current lightweight recommendation feature. It uses explainable rules based on goal priority, importance, due date, and recent task events.

**视觉与交互**
1. 每当目标或任务的状态发生变化、时间过去了30分钟或用户主动进行刷新，就要进行一次分析。
2. 推荐下一步的结果展示在 dashboard 右侧上栏（`What's Next`），当前形态为一个 recommendation card，包含一句话建议、`why` 列表，以及 `Open` 按钮。
3. What’s Next **一次只给一个推荐**（强约束）：
   - 展示形式为一句话 recommendation + short reasons.
...
**概述**
Memory is currently a manual markdown-based store for long-lived user context. It does not yet auto-extract memory from events or feed recommendations automatically.
