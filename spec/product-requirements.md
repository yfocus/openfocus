...
# OpenFocus Product Requirements
...
- OpenFocus 的当前实现形态是 Web workspace + Companion；README 中的手机端/多端统一是产品方向，当前仓库尚未实现独立手机端。
- OpenFocus 的核心不是让单个 Agent 更快，而是帮助用户管理 Goal/Task、追踪 Agent 执行、通过 Next Move 降低注意力切换成本。

### Dashboard

**概述**
Dashboard is the main workspace. It shows a goal list on the left, goal/task detail in the middle, and `Next Move` plus `Recent Events` on the right.
...
3. Dashboard 采用纵向**三栏布局**：左侧「目标列表」+ 中间「详情」+ 右侧「辅助栏」。
   ...
   - 右侧辅助栏再分上下两栏：上栏是 Next Move，下栏是 Recent Events（事件流）。
...
4. 键盘可用性（强约束）：Dashboard **整个界面**支持键盘方向键切换焦点移动。
  - `↑/↓`：在 Dashboard 内可见的可交互元素之间移动焦点（包括左侧目标/任务、中间详情按钮、右侧 Next Move / Recent Events 等）。
...
**概述**
AgentSpace is the task workspace. It binds one task to one workdir on one Companion, and gives the user a remote terminal where any command-line agent can be started. The current create dialog does not expose agent type selection; stored `agent_type=trae-cli` is a compatibility/default value rather than a product-level restriction.
...
4. 终端交互逻辑：
   ...
   - AgentSpace 右侧固定列支持 `Prompt Master` 模式：用户在文本框中输入草稿 prompt，点击 `Optimize` 后由 OpenFocus 内建 agent 调用 LLM 把草稿改写为语句流畅、意思不变的 prompt，并把优化后的 prompt 返回到该文本框/结果区。Prompt Master 必须保留用户原意与主要语言：中文或不流畅中文输入输出自然中文，英文或不流畅英文输入输出自然英文；不得把当前 Task/Goal/repo/AgentSpace 上下文、编号需求或实现细节拼进结果，除非这些信息本来就在用户草稿里。该动作只返回优化文本，不自动注入或发送到 terminal，也不创建或持久化 prompt catalog 条目。
...
**概述**
`Next Move` 是 OpenFocus 的注意力编排能力。它不是简单的单条规则推荐，而是读取上下文的推荐模块：可获取用户所有 `goals`、`tasks`、`events`、当日 `daily memory`、`MEMORY.md`、显式用户偏好，以及用户对历史推荐的反馈，并基于这些信息给出下一步建议。

**视觉与交互**
1. 每当目标或任务的状态发生变化、时间过去了30分钟或用户主动进行刷新，就要进行一次分析。
2. 推荐结果展示在 Dashboard 右侧上栏（`Next Move`），展示形态为 **最多 3 个 task recommendation cards**。
   - 每张卡至少包含：Task 标题、所属 Goal、预计耗时、任务类型、推荐理由、`Open` 按钮、`Not for now` 按钮。
   - 推荐理由必须面向人类可读，避免暴露调试字段或原始打分细节。
3. Next Move 的输入必须覆盖以下信号：
...
5. Next Move **一次最多返回 3 个推荐 task**（强约束）：
   - 不是无限列表；候选不足时允许少于 3 个。
   - 推荐之间需要有排序，但不要求强行完全不同；系统可以在“延续当前上下文”与“覆盖不同工作模式”之间做平衡。
6. 用户可以对任意一个推荐点击 `Not for now` 并填写理由。
   - 理由既支持预设选项，也支持补充自由文本。
   - 系统必须对这些反馈进行总结学习，并在后续推荐中主动规避相同问题。
7. 学习结果不能只停留在当次交互内。
   - 反馈结论必须沉淀为可复用的偏好/反偏好信号，供后续 Next Move 推荐持续读取。
8. 强约束：推荐结果中不得出现已删除、已完成、已归档、无效打开的 task。
9. 强约束：如果当前上下文不足以给出高质量推荐但存在可执行 task，也应返回候选，并在理由中明确表达不确定性；没有可执行 task 时允许为空。
...
10. README 中的 memory-to-prompt / RAG recall 是产品方向：当前用户故事只要求 Memory 页面可见、Next Move 可读取 memory、反馈学习可沉淀到 memory；独立的“召回记忆并让用户选择是否拼接到 Agent prompt”的交互不属于当前已实现范围。
